...
// Maximum number of phone numbers in the phonebook
#define MAX_PHONE_ENTRIES 15

// Define the phoneBook array with default values (will be overwritten if saved values exist)
String phoneBook[MAX_PHONE_ENTRIES] = {"+919826951787", "+919425401434", "+919425111787", "", "", "", "", "", "", "", "", "", "", "", ""};

// Function to save phonebook to non-volatile memory
void savePhoneBook() {
  preferences.begin("phonebook", false); // Open the namespace in RW mode
  
  // First, clear any existing entries
  preferences.clear();
  
  // Save the count of valid entries
  int validCount = 0;
  ...
  preferences.putInt("count", validCount);
  
  // Save each valid phone number
  int index = 0;
  for (int i = 0; i < MAX_PHONE_ENTRIES; i++) {
    if (phoneBook[i].length() > 0) {
      // Create a key string first, then convert to char*
      String keyString = "phone" + String(index);
      preferences.putString(keyString.c_str(), phoneBook[i]);
      index++;
    }
  }
  
  preferences.end();
  SerialMon.println("PhoneBook saved to non-volatile memory");
}

// Function to load phonebook from non-volatile memory
void loadPhoneBook() {
  preferences.begin("phonebook", true); // Open the namespace in read-only mode
  
  // Get the count of saved entries
  int count = preferences.getInt("count", 0);
  
  if (count > 0) {
    // Clear the default phonebook
    for (int i = 0; i < MAX_PHONE_ENTRIES; i++) {
      phoneBook[i] = "";
    }
    
    // Load saved phone numbers
    for (int i = 0; i < count && i < MAX_PHONE_ENTRIES; i++) {
      // Create a key string first, then convert to char*
      String keyString = "phone" + String(i);
      phoneBook[i] = preferences.getString(keyString.c_str(), "");
    }
    
    SerialMon.println("PhoneBook loaded from non-volatile memory");
  } else {
    SerialMon.println("No saved phonebook found, using defaults");
  }
...
void setup() {
  ...
  // Load phonebook from non-volatile memory
  loadPhoneBook();
...
void parseFormData(String data) {
  ...
  if (isDirUpdateCommand && numberCount > 0) {
    ...
    for (int i = 0; i < numberCount && i < MAX_PHONE_ENTRIES; i++) {
    ...
    // Save the updated phonebook to non-volatile memory
    savePhoneBook();
